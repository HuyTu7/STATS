...
NDF3 <- NDF %>%
  select(HappinessLevel, PhysicalAbuseAttempts)
```
...
###Chi-Square Test
```{r}
NDF4 <- data.frame(NDF3$PhysicalAbuseAttempts, NDF3$HappinessLevel)
NDF4 <- NDF4 %>%
  rename(PhysicalAbuseAttempts = NDF3.PhysicalAbuseAttempts, HappinessLevel = NDF3.HappinessLevel)
summary(DF)
NDF3$PhysicalAbuseAttempts <- factor(ifelse(NDF3$PhysicalAbuseAttempts %in% c("1", "2", "3", "4", "5", "6", "7"), "1", "0"))
NDF3$HappinessLevel <- factor(ifelse(NDF3$HappinessLevel %in% c("2", "3"), "1", "0"))
NDF4$HappinessLevel <- factor(ifelse(NDF4$HappinessLevel %in% c("2", "3"), "1", "0"))
...
```{r}
chisq.test(T2[, c(0, 2)], correct = FALSE)
chisq.test(T2[, c(0, 1)], correct = FALSE)
chisq.test(T2[, c(0, 3)], correct = FALSE)
chisq.test(T2[, c(0, 4)], correct = FALSE)
chisq.test(T2[, c(0, 5)], correct = FALSE)
chisq.test(T2[, c(0, 6)], correct = FALSE)
chisq.test(T2[, c(0, 7)], correct = FALSE)
...
Post hoc comparisons of happiness level and physical abuse attempts frequency categories revealed that higher rates of nicotine dependence were seen among those smoking more cigarettes, up to 11 to 20 times in the last year of the relationship. After the above process, it was found that there were 8 significant results including: 
...
-"this has not happened in the past year, but it did happen before then" and "once in the last year of the relationship" (Column 1 and 2) 

-"this has not happened in the past year, but it did happen before then" and "twice in the last year of the relationship" (Column 1 and 3) 

-"this has not happened in the past year, but it did happen before then" and "3 to 5 times in the last year of the relationship" (Column 1 and 4) 

-"this has not happened in the past year, but it did happen before then" and "6 to 10 times in the last year of the relationship" (Column 1 and 5) 

-"this has not happened in the past year, but it did happen before then" and "11 to 20 times in the last year of the relationship" (Column 1 and 6) 
