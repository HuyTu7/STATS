...
NDF <- addhealth_public4 %>%
  rename(TimeSpent = h4rd2y, NightsSpent = h4rd5, SatisfactionWConflict = h4rd7b, HappinessLevel = h4rd9, PhysicalAbuseAttempts = h4rd18, PhysicalInjure = h4rd20, SexualAssaults = h4rd21, PeopleMarried = h4tr1) %>%
  select(TimeSpent, NightsSpent, SatisfactionWConflict, HappinessLevel, PhysicalAbuseAttempts, PhysicalInjure, SexualAssaults, PeopleMarried)
summary(NDF)
...
My codebook has significantly more categorical variables than quantitative variables. The ones I found that are relavant to my research only produced 58 data points generated as a result which indicates a weak relationship. However, the scatterplot did suggest a relationship that is positive.
